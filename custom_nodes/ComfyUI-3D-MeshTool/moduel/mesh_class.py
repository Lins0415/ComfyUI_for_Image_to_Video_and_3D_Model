# original author https://github.com/MrForExample
...
from .image_utils import prepare_torch_img
...
class Mesh:
    ...
    def __init__(
        self,
        name: Optional[str] = None, #2024-0627 mesh names
        ...
        SmoothGroup: Optional[dict] = None,      #2024-0627 smooth group
        Transformation: Optional[Tensor] = None, #2024-0627 transformation matrix
        Metadata: Optional[dict] = None,         #2024-0628 Other data attached to the grid
        ...
        """
        self.name = name#Saving files and generating 3D scenes will require
        self.device = device
        self.v = v
        self.vn = vn
        self.vt = vt
        ...
        self.fn = fn
        self.ft = ft
        self.vc = vc# will first see if there is vertex color to use
        self.SmoothGroup = SmoothGroup
            #Structure: {s: Group1, f: [Vertex number n1,...], s: Group2, f: [Vertex number m1,...],...}
            #After an s, there can be multiple f's
        self.Transformation = Transformation#The transformation matrix attached to the grid, which can be converted into rotation and scaling
        self.Metadata = Metadata#Used to store PBRs of different standards, such as UE/Unity/VRay, etc
            #Structure: {Software Name 1: data1, Software Name 2: data2...}
            #Data or custom information within other software
        self.albedo = albedo# only support a single albedo image, (H, W, 3)
    ...
    @classmethod
    def load(cls, path, resize=True, clean=False, renormal=True, retex=False, bound=1, front_dir='+z', **kwargs):
        """load mesh from path.
    ...
    def auto_normal(self):
        ...
        vn = torch.zeros_like(self.v)# Splat face normals to vertices
