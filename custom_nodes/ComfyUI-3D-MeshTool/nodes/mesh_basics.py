    ...
    def load_mesh(self, mesh_file_path, reload):
        ...
        if not os.path.isabs(mesh_file_path):
            mesh_file_path = os.path.join(
                comfy_paths.input_directory, mesh_file_path)
        if os.path.exists(mesh_file_path):
            ...
            print(
                f"[{self.__class__.__name__}] File {mesh_file_path} does not exist").error.print()
        return (mesh, )
...
class mesh_data_get:
    ...
    CATEGORY = CATEGORY_str1+CATEGORY_str2
    RETURN_TYPES = ("MESH", "LIST", "LIST", "LIST",
                    "LIST", "LIST", "IMAGE", "IMAGE")
    RETURN_NAMES = ("mesh", "vertex", "face", "Normal_vnfn", "UVs_vtft",
                    "vertex_color", "Texture_Diffuse", "Texture_reflection")
    FUNCTION = "mesh_data"
...
class mesh_data_set:
    ...
    def mesh_data_set1(self,
        ...
        if Normal_vnfn != None:
            mesh.vn = torch.tensor(
                Normal_vnfn[0], dtype=torch.float32, device=device)
            mesh.fn = torch.tensor(
                Normal_vnfn[1], dtype=torch.int64, device=device)
        if UVs_vtft != None:
            mesh.vt = torch.tensor(
                UVs_vtft[0], dtype=torch.float32, device=device)
            mesh.ft = torch.tensor(
                UVs_vtft[1], dtype=torch.int64, device=device)
        if vertex_color != None:
            mesh.vc = torch.tensor(
                vertex_color, dtype=torch.float32, device=device)
...
class mesh_data_Statistics:
    ...
    def INPUT_TYPES(s):
        return {"required": {"mesh": ("MESH",), }, }
    CATEGORY = CATEGORY_str1+CATEGORY_str2
    RETURN_TYPES = ("LIST", "INT", "INT", "BOOLEAN", "BOOLEAN",
                    "BOOLEAN", "BOOLEAN", "BOOLEAN", "STRING")
    RETURN_NAMES = ("Output_Group", "vertex_num", "face_num", "Normal_true", "UVs_true",
                    "vertex_color_true", "Texture_Diffuse_true", "Texture_reflection_true", "device")
    ...
    def mesh_number(self, mesh):
        ...
        Output_Group = {"vertex": v_num, "face  ": f_num, "Normal": Normal_true, "UVs   ": UVs_true, "v_color": vertex_color_true,
                        "T_Diffuse": Texture_Diffuse_true, "T_reflection": Texture_reflection_true, "mesh_device": mesh_device_true}
...
class mesh_data_bus:  # 未完成
    ...
    RETURN_TYPES = ("LIST", "LIST", "LIST", "LIST", "LIST",
                    "LIST", "LIST", "IMAGE", "IMAGE",)
    RETURN_NAMES = ("vertex[N3]", "face[N2]", "Normal_v[N3]", "Normal_f[N2]", "UVs_v[N3]",
                    "UVs_f[N2]", "vertex_color[N3]", "Texture_Diffuse", "Texture_reflection",)
    ...
    def mesh_data_bus(self, mesh, vertex, face, Normal_v, Normal_f, UVs_v, UVs_f, vertex_color, Texture_Diffuse, Texture_reflection):
        ...
        list2 = ["vertex", "face", "Normal_v",
                 "Normal_f", "UVs_v", "UVs_f", "vertex_color"]
        for i in len(list1):
            if i is not None:
                ...
                if type(i) == list:
                    mesh.v = torch.tensor(
                        vertex, dtype=torch.float32, device=mesh.device)
